...
r=6.37e6
ndays=365.
navg=672#nbumber of 0.5hours to average signal 
nhours=24.
nmin=60.
ndeg=360.
...
def compute_velocities(nstep,lons,lats,time,height):
    ...
    for i in range(len(lons)-nstep):
        ...
        if dt < (1.25*navg/(ndays*nhours)) and dt > 0.00: #based on the length of February; navg=28 days
            # in this case no correction is needed for the fact that measuring freq. is half an hour
...
end=2019 #ending year balance data
min_length=int(1190) #0.767 due to ratio of shortest month to year
#correlates summer and annual velocity, given the site name, *96 because other data is averaged per 96 h and has min length 81
...
dataaveraged1=np.column_stack((tavg,lonsavg,latsavg,zavg))
