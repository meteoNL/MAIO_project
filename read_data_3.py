...
ax2=ax1.twinx()
ax2.plot(time,height,color='red',ls=':',lw=3)
ax1.plot(time,(lons+50)*np.cos(np.mean(lats)/360*2*np.pi)*6371000*2*np.pi/360.,lw=3)
...
ax1.set_ylabel('distance (m)',color='blue')
ax2.set_ylabel('height (m)',color='red')
pl.title('Dataset S5, zoom-in')
ax1.set_xlim(2012.0,2012.2)
ax1.set_ylim(-5680,-5640)
ax2.set_ylim(440,520)
pl.grid()
pl.legend()
pl.show()
#%%selection
...
ax2.set_ylabel('height (m)',color='blue')
pl.title('Dataset S5')
pl.grid()
pl.legend()
pl.show()
...
fig, ax1 = pl.subplots(figsize=(12,8))
ax2=ax1.twinx()
ax2.plot(tavg,zavg,color='red',ls=':',lw=3)
ax1.plot(tavg,(lonsavg+50)*np.cos(np.mean(lats)/360*2*np.pi)*6371000*2*np.pi/360.,lw=3)
ax1.set_xlabel('time (yr)')
ax1.set_ylabel('distance (m)',color='blue')
ax2.set_ylabel('height (m)',color='red')
pl.title('Dataset S5')
...
pl.show()
pl.figure(figsize=(12,8))
title2=fn[:-4]+' velocity time series, '+str(navg)+' hours average'
pl.title(title2)
pl.plot(timeval,velval,lw=1)
pl.grid()
