...
#        height_i=height[i]
#        print('length t, h before if:')
#        print(len(timecom),len(height))

        if np.abs(height[i]-mean_height)/std_height > crit_std:
            height[i]=-10000.
            lats[i]=-10000.
            lons[i]=-10000.
            timecom[i]=-10000.
            n=n+1
        if height[i]==-10000.:
            val = next((index for index,value in enumerate(height[i:]) if value != -10000.), None)#dit stukje werkt niet, was een van mijn laatste pogingen. Probeerde de index te bepalen van de eerst volgende/vorige element zonder -10000.
            revheight=height[::-1]
            baval=next((index for index,value in enumerate(revheight[(len(height)-i):]) if value != -10000.), None)
            height[i]=0.5*height[val]+0.5*height[baval]
            lons[i]=0.5*lons[val]+0.5*lons[baval]
            lats[i]=0.5*lats[val]+0.5*lats[baval]
            timecom[i]=0.5*timecom[val]+0.5*timecom[baval]
#        height=height[height!=-10000.]
#        lons=lons[lons!=-10000.]
#        lats=lats[lats!=-10000.]
#        timecom=timecom[timecom!=-10000.]
#        newlength=len(height)-1
#        print('length t, h after loop:')
#        print(len(timecom),len(height))
#        lons=np.interp(time,timecom,lons)#make sure that timeaxes of observed velocities and calculated velocities are equal
#        lats=np.interp(time,timecom,lats)
#        height=np.interp(time,timecom,height)
#        timecom=np.interp(time,timecom,timecom)
#
#    mean_height=np.mean(height)
#    std_height=np.std(height)
    counter+=1
