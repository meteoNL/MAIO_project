...
while np.max((abs(height)-np.mean(height))/np.std(height)) > crit_std:
    print('mean/std ding is:')#Problem:  the fraction above stays constant op 15.something (crit_std=5.0), later std is NaN, nu mean/std ding is: 1.0 en dat blijft zo (bovendien staat crit_std=20.). 
    print(np.max(np.abs(height-mean_height)/std_height))
    for i in range(len(height)):#probeerde m nog een keertje te runnen voor pushen in Github, nu geeft hij opeens problemen met index (len(height)-1 ervan maken helpt niet... eerder gaf hij die foutmelding niet, deed hij opeens nadat ik crit_std heb aangepast... zou echt niet moeten...)
        height_i=height[i]
...
#        print(len(timecom),len(height))
        lons=np.interp(time,timecom,lons)
